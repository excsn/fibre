...
#[derive(Debug, Clone)]
struct MpscRendezvousBenchConfig {
  num_producers: usize,
  total_items: usize,
}
...
#[derive(Clone)]
struct MpscRendezvousBenchState;

// Extractor function used by both Sync and Async suites
fn extract_mpsc_config(combo: &AbstractCombination) -> Result<MpscRendezvousBenchConfig, String> {
...
fn setup_fn_mpsc_sync(_cfg: &MpscRendezvousBenchConfig) -> Result<(BenchContext, MpscRendezvousBenchState), String> {
  Ok((BenchContext::default(), MpscRendezvousBenchState))
}

fn benchmark_logic_mpsc_sync(
  mut ctx: BenchContext,
  state: MpscRendezvousBenchState,
  cfg: &MpscRendezvousBenchConfig,
) -> (BenchContext, MpscRendezvousBenchState, Duration) {
  // Create a fresh synchronous rendezvous channel
  let (tx, rx) = mpsc::rendezvous::rendezvous();
...
fn setup_fn_mpsc_async(
  _rt: &Runtime,
  _cfg: &MpscRendezvousBenchConfig,
) -> Pin<Box<dyn Future<Output = Result<(BenchContext, MpscRendezvousBenchState), String>> + Send>> {
  Box::pin(async move { Ok((BenchContext::default(), MpscRendezvousBenchState)) })
}
...
fn benchmark_logic_mpsc_async(
  ...
  state: MpscRendezvousBenchState,
  cfg: &MpscRendezvousBenchConfig,
) -> Pin<Box<dyn Future<Output = (BenchContext, MpscRendezvousBenchState, Duration)> + Send>> {
  let cfg_clone = cfg.clone();
  Box::pin(async move {
    // Create a fresh asynchronous rendezvous channel
    let (tx, rx) = mpsc::rendezvous::rendezvous_async();
...
fn mpsc_rendezvous_sync_benches(c: &mut Criterion) {
  ...
  .throughput(|cfg: &MpscRendezvousBenchConfig| Throughput::Elements(cfg.total_items as u64))
  .run();
}

fn mpsc_rendezvous_async_benches(c: &mut Criterion) {
...
criterion_group!(benches, mpsc_rendezvous_sync_benches, mpsc_rendezvous_async_benches);
