...
Enable the `diagnostics` feature if you want telemetry output during TSan runs.
...
*   `--features diagnostics`: To enable `fibre`'s internal telemetry for detailed logs if the test includes telemetry calls.
...
1.  **Isolate the Problematic Test:** Use `nextest` to run only the specific `fibre` test configuration (e.g., a specific SPMC consumer/capacity combination) that hangs or is suspect.
    ...
    cargo nextest run spmc_deadlock_tests::looped_repro_spmc_sync_hang_4c_1cap --features diagnostics
    ```
