//! Multi-producer, single-consumer **rendezvous** channel.
//!
...
//! blocking and async ends are provided and interoperate. See
...
use crate::internal::rendezvous::{MpscRvShared, WAITING};
...
/// Creates a synchronous MPSC rendezvous channel.
pub fn rendezvous<T: Send>() -> (RendezvousSyncSender<T>, RendezvousSyncReceiver<T>) {
  let shared = Arc::new(MpscRvShared::new());
  (
...
/// Creates an asynchronous MPSC rendezvous channel.
...
/// The synchronous sending half of an MPSC rendezvous channel. Cloneable.
#[derive(Debug)]
...
/// The asynchronous sending half of an MPSC rendezvous channel. Cloneable.
#[derive(Debug)]
...
impl<T: Send> RendezvousSyncSender<T> {
  ...
  /// Closes this handle. If it is the last sender, a parked receiver observes
  /// disconnection.
...
impl<T: Send> RendezvousSyncReceiver<T> {
  ...
  /// Closes this handle; parked senders observe closure.
  pub fn close(&self) -> Result<(), CloseError> {
    if self
      .closed
      .compare_exchange(false, true, Ordering::AcqRel, Ordering::Relaxed)
...
impl<'a, T: Send> SendFuture<'a, T> {
  fn new(shared: &'a Arc<MpscRvShared<T>>, item: T) -> Self {
    Self {
      shared,
      slot: Some(item),
...
impl<'a, T: Send> RecvFuture<'a, T> {
  fn new(shared: &'a Arc<MpscRvShared<T>>) -> Self {
    Self {
      shared,
      dest: None,
      state: AtomicU8::new(WAITING),
