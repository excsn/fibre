...
Cross-implementation channel benchmarks: fibre against tokio, crossbeam-channel, flume, kanal, async-channel and `std::sync::mpsc`, through one shared workload harness.
...
| Axis | Values |
| :--- | :--- |
| Shape | spsc, mpsc, spmc (broadcast), mpmc |
| Mode | sync, async |
...
Full matrix is 224 cells, +/- five minutes.
...
| File | Role |
| :--- | :--- |
| `src/spec.rs` | matrix vocabulary: shape, mode, capacity, pairing, api, cell |
| `src/channel.rs` | the two adapter traits and the fan-out helpers |
...
| `src/adapters/` | one file per library |
