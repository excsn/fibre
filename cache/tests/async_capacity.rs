...
async fn test_async_insert_is_non_blocking_and_janitor_cleans_up() {
  ...
  assert!(cache.fetch(&1).await.is_some(), "Item 1 should still be present");
  assert!(
    cache.fetch(&2).await.is_some(),
  ...
  assert!(cache.fetch(&1).await.is_none(), "Item 1 should have been evicted");
  assert!(cache.fetch(&2).await.is_none(), "Item 2 should have been evicted");
...
async fn test_async_janitor_evicts_on_capacity_with_lru() {
  ...
  assert!(cache.fetch(&0).await.is_none(), "Key 0 should have been evicted");
  for i in 1..=10 {
    assert!(cache.fetch(&i).await.is_some(), "Key {} should still be present", i);
  }
  assert_eq!(cache.metrics().evicted_by_capacity, 1);
}
...
async fn test_async_janitor_evicts_on_capacity_with_default_tinylfu() {
  ...
  let item1_present = cache.fetch(&1).await.is_some();
  let item2_present = cache.fetch(&2).await.is_some();
  let item3_present = cache.fetch(&3).await.is_some();
...
async fn test_async_no_eviction_if_at_capacity() {
  ...
  for i in 0..cache_capacity {
    assert!(
      cache.fetch(&(i as i32)).await.is_some(),
...
async fn test_async_janitor_cleans_up_large_overflow() {
  ...
  assert!(cache.fetch(&2).await.is_some());
  let evictions_before_janitor = cache.metrics().evicted_by_capacity;
