...
impl LogHandler {
  ...
  /// Converts a `log::Record` into our internal `LogEvent` format.
  fn build_log_event(&self, record: &Record<'_>) -> LogEvent {
...
impl log::Log for LogHandler {
  ...
  fn log(&self, record: &Record) {
    ...
    let log_event = self.build_log_event(record);

    // Create the required `tracing::Metadata` on the fly.
    let level = log_event.level;
    ...
    self.processor.process_event(log_event, &metadata);
  }
