...
*   **`Sender` / `Producer`:** These handles are used to send data into a channel. Depending on the channel type, they may be clonable to allow for multiple producers.
*   **`Receiver` / `Consumer`:** These handles are used to receive data from a channel. Depending on the channel type, they may be clonable to allow for multiple consumers.
