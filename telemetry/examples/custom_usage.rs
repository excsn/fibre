...
use fibre_telemetry::{init, InternalErrorSource};
use log::info;
use std::fs;
use std::path::Path;
use std::thread;
...
fn main() {
  ...
  println!("\n--- Initializing fibre_telemetry ---");
  let config_path = Path::new(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/examples/fibre_telemetry.advanced.yaml"
  ));
  let mut init_result =
    init::init_from_file(config_path).expect("Failed to initialize fibre_telemetry");
  ...
  if let Some(metrics_rx) = init_result.custom_streams.remove("metrics_stream") {
    ...
    thread::spawn(move || {
      ...
      match metrics_rx.recv() {
        Ok(event) => {
          println!("\n>>> [Metrics Consumer] Received TelemetryEvent >>>");
